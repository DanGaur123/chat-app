  ...
  return (
    <Stack direction={"row"} justifyContent={el.incoming ? "flex-start" : "flex-end"}>
      <Box p={1.5} sx={{ backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content" }}>
       <Stack spacing={2}>
          <Stack direction={"row"} p={2} spacing={5} alignItems={"center"} sx={{backgroundColor:theme.palette.background.paper,borderRadius:1}}>
            <Image size={48} />
...
const LinkMsg = ({el,menu}) => {
  ...
  return (
    <Stack direction={"row"} justifyContent={el.incoming ? "flex-start" : "flex-end"}>
      <Box p={1.5} sx={{ backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content" }}>
      <Stack spacing={2}>
       <Stack p={2} spacing={3} alignItems={"start"} sx={{backgroundColor:theme.palette.background.paper,borderRadius:1.5}}>
         <img src={el.preview} alt={el.message} style={{maxHeight:210, borderRadius:"10px"}}/>
       <Stack spacing={2}>
       <Typography variant='subtitle2'>Hello this is it</Typography>
       <Typography variant='subtitle2' sx={{color:theme.palette.primary.main}} component={Link} to="www.youtube.com">www.youtube.com</Typography>
       </Stack>
...
const ReplyMsg = ({ el,menu }) => {
  ...
  return (
    <Stack direction={"row"} justifyContent={el.incoming ? "flex-start" : "flex-end"}>
      <Box p={1.5} sx={{ backgroundColor: el.incoming ? theme.palette.background.default : theme.palette.primary.main, borderRadius: 1.5, width: "max-content" }}>
       <Stack spacing={2}>
        <Stack p={2} direction={"column"} alignItems={"center"} spacing={3} sx={{backgroundColor:theme.palette.background.paper,borderRadius:1.5}}>
